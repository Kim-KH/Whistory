...
import { getTopic, starsFor, TOPIC_CATEGORIES } from '../content/registry';

const NARRATIVE_STAGES = [
  { key: 'cause', label: '원인·배경' },
  { key: 'development', label: '전개' },
  { key: 'crisis', label: '위기·전환점' },
  { key: 'conclusion', label: '결과·의의' },
];
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <AppText style={s.era}>
          ...
          {topic.yearStart ? ` · ${topic.yearStart}${topic.yearEnd && topic.yearEnd !== topic.yearStart ? `~${topic.yearEnd}` : ''}` : ''}
        </AppText>
        ...
        {relatedTopics.length > 0 && (
          <>
            <View style={s.divider} />
            <AppText style={s.blockTitle}>관련 주제</AppText>
